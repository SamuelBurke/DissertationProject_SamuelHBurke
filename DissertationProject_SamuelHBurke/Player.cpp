...
void Player::Init(std::shared_ptr<Foundations::Transform> _transform)
{
	...
	m_playerPos = glm::vec3(0.0f, 0.0f, -7.0f);
...
void Player::Update(float _deltaTime)
{
	...
	if (m_input.isKeyPressed(SDL_SCANCODE_W) || m_input.isKeyPressed(SDL_SCANCODE_UP))    { m_direction = glm::vec3( 0.0f,  0.0f,-1.0f); }

	if (m_input.isKeyPressed(SDL_SCANCODE_S) || m_input.isKeyPressed(SDL_SCANCODE_DOWN))  { m_direction = glm::vec3( 0.0f,  0.0f, 1.0f); }
